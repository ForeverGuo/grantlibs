...
function cloneOtherType(obj: any, type: string) {
  switch (type) {
    case boolType:
    case numType:
    case strType:
...
// 深拷贝
export function deep(obj: any, map = new Map()): any {
  // 如果不是对象直接返回
  ...
  if (objType === mapType) {
    obj.forEach((val: any, key: any) => {
      obj_.set(key, deep(val, map))
    })
    return obj_
  }
  // 如果是数组 或者 {}
  for (let i in obj) {
    obj_[i] = deep(obj[i], map)
  }
